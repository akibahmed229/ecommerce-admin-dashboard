...
export class RoleRepository implements IRoleRepository {
    private async getPermissions(roleId: string, executor: any = db) {
        return executor
            .select({ id: permissionsTable.id, groupId: permissionsTable.groupId, name: permissionsTable.name, description: permissionsTable.description, createdAt: permissionsTable.createdAt, updatedAt: permissionsTable.updatedAt })
    ...
    private async userCount(roleId: string, executor: any = db) {
        const [{ value }] = await executor.select({ value: count() }).from(usersTable).where(eq(usersTable.roleId, roleId));
        return value;
    }
    ...
    async findById(id: string): Promise<RoleWithPermissions | null> {
        ...
        if (!role) return null;
        return { ...role, permissions: await this.getPermissions(id), userCount: await this.userCount(id) };
    }
    ...
    async findAll(options: PaginationOptions): Promise<PaginatedResult<RoleWithPermissions>> {
        ...
        const rows = await db.select().from(rolesTable).where(whereClause).limit(limit).offset(offset);
        const [{ value: total }] = await db.select({ value: count() }).from(rolesTable).where(whereClause);

        const data: RoleWithPermissions[] = [];
        for (const role of rows) {
            data.push({ ...role, permissions: await this.getPermissions(role.id), userCount: await this.userCount(role.id) });
        }
    ...
    async create(input: CreateRoleInput): Promise<RoleWithPermissions> {
        return db.transaction(async (tx: any) => {
            const [role] = await tx.insert(rolesTable).values({ name: input.name, description: input.description, status: input.status ?? "active" }).returning();
            if (input.permissionIds.length) {
                await tx.insert(rolePermissionsTable).values(input.permissionIds.map((permissionId) => ({ roleId: role.id, permissionId })));
            }
    ...
    async update(id: string, input: UpdateRoleInput): Promise<RoleWithPermissions> {
        return db.transaction(async (tx: any) => {
            const [existing] = await tx.select().from(rolesTable).where(eq(rolesTable.id, id));
            if (!existing) throw new NotFoundError("Role not found");

            if (input.name !== undefined || input.description !== undefined || input.status !== undefined) {
                await tx.update(rolesTable).set({ name: input.name, description: input.description, status: input.status, updatedAt: new Date() }).where(eq(rolesTable.id, id));
            }
            if (input.permissionIds) {
                await tx.delete(rolePermissionsTable).where(eq(rolePermissionsTable.roleId, id));
                if (input.permissionIds.length) {
                    await tx.insert(rolePermissionsTable).values(input.permissionIds.map((permissionId) => ({ roleId: id, permissionId })));
                }
            }

            const [role] = await tx.select().from(rolesTable).where(eq(rolesTable.id, id));
            return { ...role, permissions: await this.getPermissions(id, tx), userCount: await this.userCount(id, tx) };
        });
    }
