...
export class MediaRepository implements IMediaRepository {
    ...
    async checkUsage(id: string): Promise<{ isAttached: boolean; attachedToModule?: string }> {
        const [inBrand] = await db
            .select({ id: brandsTable.id })
            .from(brandsTable)
            .where(eq(brandsTable.logoId, id)).limit(1);
        ...
        const [inCategory] = await db
            ...
            .where(eq(categoriesTable.imageId, id)).limit(1);

        if (inCategory) return { isAttached: true, attachedToModule: "category" };

        const [inAttrValue] = await db
            .select({ id: attributeValuesTable.id })
            .from(attributeValuesTable)
            .where(eq(attributeValuesTable.mediaId, id))
            .limit(1);
